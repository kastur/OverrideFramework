...
/**
 * The backend service class that manages LocationProviders and issues location updates and alerts
 * to listeners. <p/> Bare-bones version of the LocationManagerService from the Android Framework.
 * Here, we allow arbitrary, external location providers to call reportLocation. These locations are
 * transported to the appropriate listeners.
 */
public class OverrideLocationServiceImpl extends IOverrideLocationService.Stub {
  ...
  private final
  HashMap<String, ArrayList<UpdateRecord>>
      mRecordsByProvider =
  ...
  public void requestLocationUpdates(String packageName, String provider, boolean singleShot,
                                     IOverrideLocationListener listener) {
    try {
      synchronized (mLock) {
  ...
  private void requestLocationUpdatesLocked(String packageName, String native_provider,
    ...
    String provider = "RandomWalkLocationProvider";

    if (LOCAL_LOGV) {
      Log.i(TAG,
            "requestLocationUpdatesLocked: registering a location updates for: " + "\"" + provider
            + "\"");
    }
